...
import xiangshan.{MicroOp, XSBundle}
...
class FmaFeedbackBundle(implicit p: Parameters) extends XSBundle{
  val pdest = UInt(PhyRegIdxWidth.W)
  val midResult = new FMAMidResult
}
class FMAMidResultIO(implicit p: Parameters) extends Bundle {
