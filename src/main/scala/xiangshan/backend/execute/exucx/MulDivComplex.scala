...
class MulDivComplex(id: Int, bypassNum:Int)(implicit p:Parameters) extends BasicExuComplex {
  val mul = LazyModule(new MulExu(id, "MulDivStdComplex", bypassNum))
  val div = LazyModule(new DivExu(id, "MulDivStdComplex", bypassNum))
  ...
  lazy val module = new MulDivComplexImp(this, id, bypassNum)
}

class MulDivComplexImp(outer:MulDivComplex, id:Int, bypassNum:Int) extends BasicExuComplexImp(outer, bypassNum){
    ...
    issueIn.issue.ready := Mux(issueIn.issue.bits.uop.ctrl.fuType === FuType.div, issueDiv.issue.ready, issueMul.issue.ready)
