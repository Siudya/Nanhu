...
class RVCExpander(implicit p: Parameters) extends XSModule {
  ...
  if (HasCExtension) {
    io.out := new RVCDecoder(io.in, XLEN, XLEN, true).decode
  } else {
    io.out := new RVCDecoder(io.in, XLEN, XLEN, true).passthrough
  }
}
...
class PredChecker(implicit p: Parameters) extends XSModule with HasPdConst {
  ...
  val jumpTargets          = VecInit(pds.zipWithIndex.map{case(pd,i) => ZeroExt((pc(i) + jumpOffset(i))(VAddrBits - 1, 0), XLEN)})
